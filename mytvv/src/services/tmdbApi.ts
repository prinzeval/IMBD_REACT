...
export const fetchTrendingAll = async (timeWindow: string = 'day', language: string = 'en-US', page: number = 1) => {
  return await fetchFromTMDB(`/trending/all/${timeWindow}`, { language, page });
};

// 2. Popular Movies - GET /movie/popular
export const fetchPopularMovies = async (page: number = 1, language: string = 'en-US', region: string = 'US') => {
  const params = { language, page, region };
  return await fetchFromTMDB('/movie/popular', params);
};

// 3. Popular TV - GET /tv/popular
export const fetchPopularTV = async (page: number = 1, language: string = 'en-US') => {
  return await fetchFromTMDB('/tv/popular', { language, page });
};

// 4. Now Playing Movies - GET /movie/now_playing
export const fetchNowPlayingMovies = async (page: number = 1, language: string = 'en-US', region: string = 'US') => {
  const params = { language, page, region };
  return await fetchFromTMDB('/movie/now_playing', params);
};

// 5. Movie Genres - GET /genre/movie/list
export const fetchMovieGenres = async (language: string = 'en') => {
  return await fetchFromTMDB('/genre/movie/list', { language });
};

// 6. TV Genres - GET /genre/tv/list
export const fetchTVGenres = async (language: string = 'en') => {
  return await fetchFromTMDB('/genre/tv/list', { language });
};

// Get movie/TV details
export const fetchMovieDetails = async (id: string, language: string = 'en-US') => {
  return await fetchFromTMDB(`/movie/${id}`, { language, append_to_response: 'release_dates' });
};

export const fetchTVDetails = async (id: string, language: string = 'en-US') => {
  return await fetchFromTMDB(`/tv/${id}`, { language });
};

// Search
export const searchContent = async (query: string, page: number = 1, language: string = 'en-US') => {
  return await fetchFromTMDB('/search/multi', { query, page, language });
};

// Advanced Discover API functions with country filtering
export const discoverMovies = async (filters: Record<string, any> = {}) => {
  const {
    page = 1,
    language = 'en-US',
    region = 'US',
...
export const discoverTV = async (filters: Record<string, any> = {}) => {
  const {
    page = 1,
    language = 'en-US',
    region = 'US',
...
export const fetchMoviesByCountry = async (countryCode: string, page: number = 1, language: string = 'en-US') => {
...
export const fetchTVByCountry = async (countryCode: string, page: number = 1, language: string = 'en-US') => {
...
export const fetchLatestMoviesByCountry = async (countryCode: string, page: number = 1, language: string = 'en-US') => {
...
export const fetchNowPlayingByCountry = async (countryCode: string, page: number = 1, language: string = 'en-US') => {
  return await discoverMovies({
    page,
    language,
    region: countryCode,
...
export const fetchContentByGenre = async (mediaType: string, genreId: string, page: number = 1, language: string = 'en-US') => {
  const params = {
    language,
    page,
    with_genres: genreId,
...
export const fetchRecommendations = async (mediaType: string, id: string, page: number = 1, language: string = 'en-US') => {
  return await fetchFromTMDB(`/${mediaType}/${id}/recommendations`, { language, page });
};
